...
## 🗓️ 2026-04-15: Core UX & Tactical Layout Renewal

### ✅ 주요 성과
- **메인 페이지 (Overview)**: 플랩풋볼 스타일의 날짜별 그룹화 리스트로 리뉴얼. 정보 밀도 강화.
- **전술판 레이아웃**: 상단 경기장, 하단 가로형 대기 명단 구조로 개편하여 수직 전술 동선 확보.
- **DnD (드래그 앤 드롭) 복구**: DragOverlay 및 PointerSensor 최적화로 부드러운 드래그 경험 제공. 사이드바는 정보용, 대기 명단은 배치용으로 역할 분리.
- **마이페이지 (My Page)**: 내가 생성한 매치 이력을 관리할 수 있는 신규 페이지 도입.
- **데이터 안정성**: 포메이션 변경 시 사라진 선수가 자동으로 대기 명단으로 복구되는 로직 구현.
- **보안 개방**: 초대 링크를 통한 게스트의 선수 등록 기능 정상화 (POST 401 에러 해결).


## 🗓️ 2026-04-15 (PM): Layout Optimization & Workspace Mode

### ✅ 작업 내용
- **상세 페이지 헤더 제거**: 매치 관리 집중을 위해 불필요한 Topbar 제거 및 전체 레이아웃 정돈.
- **UI/UX 개선**: 
  - 쿼터 선택 탭과 포메이션 선택기를 헤더 영역에 인라인 배치하여 동선 최적화.
  - 상세 페이지 레이아웃 수직 스택(경기장 상단, 가로형 대기 명단 하단) 구현.
  - Empty State 및 모바일 UI 대응 정렬 수정.
- **빌드 에러 해결**: Next.js 빌드 시 발생하던 타입 에러 및 컴포넌트 임포트 누락 건 해결.
